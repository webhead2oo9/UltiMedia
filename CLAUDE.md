...
- **Renderer:** `media_renderer` (`Software` | `OpenGL`, default `Software`) — `SET_HW_RENDER` is load-time-only, so changes apply at the next content load; if the frontend refuses GL (e.g. a d3d video driver), the core silently stays on software
