...
- **Renderer:** `media_renderer` (`Software` | `OpenGL`, default `Software`) — `SET_HW_RENDER` is load-time-only, so changes apply at the next content load; if the frontend refuses GL (e.g. a d3d video driver), the core silently stays on software. Once negotiation succeeds, the video callback only ever gets `RETRO_HW_FRAME_BUFFER_VALID` or `NULL` (frame dupe) — never the CPU framebuffer, which 1.7.5's gl driver would ignore in hw mode
